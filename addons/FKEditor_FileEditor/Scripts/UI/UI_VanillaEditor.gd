...
var Preview = load("res://addons/FKEditor_FileEditor/Scenes/Preview.tscn")
...
func color_region(filextension : String): 
	# -----------------------------> dal momento che voglio creare un editor per ogni file, 
	# renderò questa funzione singola in base all'estensione del file
	match(filextension):
		"bbs":
